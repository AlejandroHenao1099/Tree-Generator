...
public class CellularPlusNoiseTerrain : MonoBehaviour
{
    [Header("General")]
    ...
    public float amplitudeCellular = 1;
    ...
    private void ActualizarMapa()
    {
        for (int y = 0; y < resolucion; y++)
        {
            ...
            {
                float value = (fastNoise.GetNoise(x + posX, y + posY) * 0.5f + 0.5f) * amplitude;
                value += (fastNoiseCellular.GetNoise(x + posCellular.x, y + posCellular.y) * 0.5f + 0.5f) * amplitudeCellular;
                heightMap[y, x] = value;
            }
        }
        UpdateVertices();
