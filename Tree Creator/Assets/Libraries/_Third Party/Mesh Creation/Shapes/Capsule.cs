...
namespace MeshGenerator
{
    public static class Capsule
    {
        ...
        public static Mesh Create(int resolution, float radius, float height, int resolutionVertical = 1)
        {
            Capsule.radius = radius;
            Capsule.resolution = resolution;
            Capsule.height = height;
            Capsule.resolutionVertical = resolutionVertical;
            var mesh = new Mesh();
            mesh.name = "Capsule";
        ...
        private static void CreateVertices()
        {
            ...
            vertices = new Vector3[sizeVertices + (lenghtRing * (resolutionVertical - 1))];
            int vi = 0;
            if (resMin == 3)
            ...
            int yMidle = resMin == 2 ? resolution / 2 : Mathf.CeilToInt(resolution / 2) + 1;
            float heightBody = height - (radius * 2f);
            float stepHeightBody = heightBody / (float)resolutionVertical;
                ...
                if (y == yMidle)
                {
                    for (int i = 0; i < resolutionVertical - 1; i++)
                    {
                        for (int x = 0; x < resolution; x++)
                            SetBodyVertex(vi++, x, i, 0);
                        for (int z = 1; z < resolution; z++)
                            SetBodyVertex(vi++, resolution - 1, i, z);

                        for (int x = resolution - 2; x >= 0; x--)
                            SetBodyVertex(vi++, x, i, resolution - 1);
                        for (int z = resolution - 2; z > 0; z--)
                            SetBodyVertex(vi++, 0, i, z);
                    }
                }
                for (int x = 0; x < resolution; x++)
                    SetVertex(vi++, x, y, 0);
        ...
        private static void SetVertex(int i, int x, int y, int z)
        {
            ...
            var vertice = s * radius;
            var offset = new Vector3(0, (height / 2f) - radius, 0);
            vertices[i] = vertice.y > 0 ? vertice + offset : vertice - offset;
            // cubeUV[i] = new Color32((byte)x, (byte)y, (byte)z, 0);
        }

        private static void SetBodyVertex(int i, int x, int y, int z)
        {
            float heightBody = height - (radius * 2f);
            float stepHeightBody = heightBody / (float)(resolutionVertical);
            float baseY = (-height * 0.5f) + radius + stepHeightBody;
            int lenghtRing = resolution * 2 + (resolution - 2) * 2;

            float stepRes = 2f / (float)(resolution - 1);
            Vector3 v = new Vector3(x, baseY + (y * stepHeightBody), z);
            v.x = (v.x * stepRes) - 1f;
            v.z = (v.z * stepRes) - 1f;
            Vector3 heightNormalize = Vector3.up * v.y;
            v = ((v - heightNormalize).normalized * radius) + heightNormalize;
            vertices[i] = v;
        }
        ...
        private static void CreateTriangles()
        {
            ...
            int quadsBody = (resolution - 1) * 4 * resolutionVertical;
            triangles = new int[(quads * 6) + (quadsBody * 6)];
            int ti = 0, vi = 0;
        ...
        private static void CreateLateralFaces(ref int vi, ref int ti)
        {
            ...
            for (int ver = 0; ver < resolution - 1 + resolutionVertical - 1; ver++)
            {
