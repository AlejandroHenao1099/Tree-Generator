...
public class CameraTargetOrientationScript : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (enableRotation && (Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D)))
        {
            Quaternion camAngle = Quaternion.AngleAxis(Input.GetAxis("Horizontal") * rotationSpeedMouse, Vector3.up);
        ...
        else
        {
            ...
            _zoomAmountMouse += Input.GetAxis("Vertical");
            _zoomAmountMouse = Mathf.Clamp(_zoomAmountMouse, -_maxToClampMouse, _maxToClampMouse);

            var translate = Mathf.Min(Mathf.Abs(Input.GetAxis("Vertical")), _maxToClampMouse - Mathf.Abs(_zoomAmountMouse));
            transform.Translate(0, 0, translate * zoomSpeedMouse * Mathf.Sign(Input.GetAxis("Vertical")));
